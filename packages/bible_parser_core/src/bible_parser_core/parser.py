...
ASR_REPLACEMENTS = (
    ...
    (r"\b(?:книга|пророка)?\s*со\s+фон\s+и\s+я\b", "софония"),
    (r"\bивреем\b", "евреям"),
    (r"\bне\s+ем\s+и\s+я\b", "неемия"),
    (r"\bниэмии\b", "неемии"),
    (r"\bниемии\b", "неемии"),
    ...
    (r"\bкол\s+ось\s+яна\b", "колоссянам"),
    (r"\bко\s+лось\s+яна\b", "колоссянам"),
    (r"\bкол\s+сям\b", "колоссянам"),
    ...
    (
        r"\b(?:2|второе|вторая|второго)\s+(?:послани[ея]\s+)?фес+с?\s+салон(?:ик|ики)?\b",
        "2 фессалоникийцам",
    ),
...
def replace_number_phrases(text: str) -> str:
    ...
    for token in text.split():
        if (
            token in NUMBER_WORD_BLACKLIST
            or token in NUMBER_WORDS
            or not re.fullmatch(r"[а-яa-z]+", token)
        ):
