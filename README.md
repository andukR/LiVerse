...
например `Иоанн 3:16-36`.

Межглавные диапазоны Holyrics Bible module не показывает как единый диапазон,
поэтому LiVerse обходит это ограничение через `ShowQuickPresentation`:
...
`HOLYRICS_QUICK_MINUTES` для межглавных диапазонов не применяется, чтобы
